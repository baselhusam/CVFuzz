...
copyButton?.addEventListener("click", async () => { try { await navigator.clipboard.writeText("cvfuzz run model.pt street.mp4"); copyButton.querySelector("span").textContent = "Copied"; window.setTimeout(() => { copyButton.querySelector("span").textContent = "Copy"; }, 1500); } catch { copyButton.querySelector("span").textContent = "Select"; } });
