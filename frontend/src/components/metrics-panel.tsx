    ...
    <div className="overflow-hidden border border-[#24303b] bg-[#070b10] shadow-[inset_0_1px_0_rgba(255,255,255,.035)]">
      <div className="relative cursor-crosshair px-2 pt-2 sm:px-4" onMouseMove={(event) => {
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="block w-full" role="img" aria-label={kind === "retention" ? "Detection retention over the stream" : "Failure events per sampled frame"}>
          ...
          {hoverIndex != null && <><line x1={pointX(hoverIndex)} x2={pointX(hoverIndex)} y1={top} y2={height - bottom} stroke="#e9f1f5" strokeWidth="1" strokeDasharray="3 4" opacity=".7" />{plottedItems.map((item) => <circle key={item.id} cx={pointX(hoverIndex)} cy={pointY(valueAt(item, hoverIndex))} r={item === focus ? "4" : "2.6"} fill={SERIES_COLORS[metrics.transforms.indexOf(item) % SERIES_COLORS.length]} stroke="#070b10" strokeWidth="1.5" opacity={focus && item !== focus ? ".4" : "1"} />)}</>}
        ...
        {hoverIndex != null && hoverPosition && <div className="pointer-events-none absolute z-10 w-[210px] overflow-hidden border border-[#52616d] bg-[#0a1118]/95 px-3 py-2 shadow-[0_14px_34px_rgba(0,0,0,.55)] backdrop-blur-sm" style={{ left: Math.max(8, Math.min(hoverPosition.x + 14, hoverPosition.width - 218)), top: Math.max(8, Math.min(hoverPosition.y - 18, hoverPosition.height - tooltipHeight - 8)) }}><div className="mb-1.5 flex items-center justify-between border-b border-[#263440] pb-1.5 font-mono text-[8px] uppercase tracking-[.1em] text-[#9babb7]"><span>Sample {formatTime(hoverTime)}</span><span>{focus ? "Focused" : `${tooltipItems.length} streams`}</span></div><div className="space-y-1 font-mono text-[9px] leading-4">{tooltipItems.map((item) => { const value = valueAt(item, hoverIndex); return <div key={item.id} className="flex items-center justify-between gap-3"><span className="flex min-w-0 items-center gap-1.5 text-[#c4ced4]"><i className="size-1.5 shrink-0" style={{ background: SERIES_COLORS[metrics.transforms.indexOf(item) % SERIES_COLORS.length] }} /><span className="truncate">{item.name}</span></span><span className="shrink-0 text-[#f2f5f6]">{kind === "retention" ? `${value}%` : `${value} events`}</span></div> })}</div></div>}
      </div>
      <div className="border-t border-[#24303b] bg-[#0a1016] px-4 py-3 font-mono text-[8.5px] leading-5">
        <div className="mb-1.5 flex items-center justify-between gap-3"><span className="uppercase tracking-[.12em] text-[#81909c]">{hoverIndex == null ? (focus ? `Focus · ${focus.name}` : "Stream totals") : `Sample ${formatTime(hoverTime)}`}</span><span className="hidden uppercase tracking-[.1em] text-[#5f6d78] sm:block">{hoverIndex == null ? (focus ? "Focused series renders above the comparison set" : "Select a legend item to focus") : "Live readout"}</span></div>
...
function ExpandedChart({ kind, metrics, focusedId, onFocus, onClose }: { kind: ChartKind; metrics: RunMetrics; focusedId: string | null; onFocus: (id: string | null) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink/90 p-2 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={`Expanded ${title}`} onMouseDown={onClose}>
      <div className="max-h-[92vh] w-[calc(100vw-1rem)] max-w-[1440px] overflow-y-auto border border-[#33424e] bg-[#090e14] p-3 shadow-[0_30px_100px_rgba(0,0,0,.65)] sm:w-[calc(100vw-3rem)] sm:p-6" onMouseDown={(event) => event.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between gap-5"><div className="min-w-0"><p className="section-kicker text-signal">Expanded analysis</p><h3 className="mt-2 text-xl tracking-[-.03em]">{title}</h3><p className="mt-1 text-[11px] text-[#a7b2ba]">{subtitle} Hover inside the plot for a live sampled-frame readout.</p></div><button type="button" onClick={onClose} className="flex size-9 shrink-0 items-center justify-center border border-[#33424e] text-[#a7b2ba] transition-colors hover:border-signal hover:text-foreground" aria-label="Close expanded chart"><X className="size-4" /></button></div>
